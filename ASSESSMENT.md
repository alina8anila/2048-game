...
## Підсумкова оцінка коду (2026-04-26)

### Фінальний стан коду: 24/25

| Підкритерій | Бали | Коментар |
|---|:---:|---|
| Повнота мінімальних вимог | 12/12 | Виконано 12 з 12: ✅ сітка 4x4 (`board_type EQU 4`, `main.asm:13`, `board DB 25 DUP(0)` для запасу 5x5); ✅ центровані числа (`render.asm:578-584` — `padding = (TILE_WIDTH - string_length) / 2`) і порожні клітинки відрізняються (`tile_colors[0]=00h`, чорний); ✅ 12 кольорів для значень 2..2048 (`main.asm:16-27`); ✅ зсув у 4 напрямках (`slide_left/right/up/down`, `logic.asm:128, 192, 260, 327`); ✅ злиття через `inc byte ptr [si]` у Підході B (`logic.asm:102`); ✅ одне злиття за хід через compress→merge→compress + `inc si` після merge (`logic.asm:116`); ✅ порожній хід через `compare_boards` (`logic.asm:576`) у `main.asm:175-178`; ✅ нова плитка 90/10% через два `random_range` (`logic.asm:419-430` — `cmp ax, 0` після range(10)); ✅ рахунок (`curr_score`, `best_score` у `draw_score`, `render.asm:652`); ✅ кінець гри через copy→slide×4→find0→restore (`logic.asm:482-522`); ✅ перемога через `cmp [si], byte ptr 11` (2^11=2048, `logic.asm:534`); ✅ перезапуск R через `reset_gamelog` + `jmp start` (`main.asm:137-139`). |
| Структура та модульність | 5/5 | Три модулі коду: `main.asm` (192 рядки) — глобальні дані + диспетчеризація клавіш; `logic.asm` (674) — 13 PROC ігрової логіки; `render.asm` (1049) — 9 PROC рендерингу. Плюс два тестові файли `testlog.asm` (146) і `testren.asm` (134). INCLUDE у `main.asm:190-191`. Іменовані константи EQU (`TILE_WIDTH`, `TILE_HEIGHT`, `board_type`, `*_LEN`, `HINT_0_LEN`). Таблиці організовано (`tile_colors`, `hint_table`/`HINT_LENS`, `msg_table`). Чисті PROC-кордони. |
| Коректність (стек, регістри, edge cases) | 4/5 | Стек збалансований у всіх PROC. Передача параметрів через стек з `[bp+4]`/`[bp+6]` коректна (`get_num`, `compress_row`, `copy_boards`, `compare_boards`, `random_range`, `print_line`, `draw_tile`, `num_to_str`). Callee-saves через `push`/`pop` пар. Ключовий граничний випадок `[2,2,2,2]→[4,4,0,0]` працює (compress→merge: si=0 merge→[2,0,1,1], inc si двічі → si=2 merge→[2,0,2,0], compress→[2,2,0,0] у експонентах = [4,4,0,0]). Дрібниця: `slide_down` має нестандартне форматування (`push si` без відступу `logic.asm:328`), але семантично коректний. Без захисту від overflow після 2048: `merge_row` робить `inc byte ptr [si]` без перевірки межі 11 — після continue гра може вийти за межі `tile_colors[11]`. |
| Якість коментарів | 3/3 | Header-коментар на кожній PROC (`; КТ-3`, `; collecting non-zero tiles`, `; merging neighboring same tiles`). WHY-коментарі поряд з тонкими місцями: `inc byte ptr [si] ;left++` (контекст Підходу B = подвоєння значення), `;if(si<bx) formerge`, формули `;start_row = (25 - (board_type * (TILE_HEIGHT + 1) - 1)) / 2`, ASCII-коди рамок з поясненнями (`; ║ 186, ═ 205, ╔ 201`). Після passу перекладу 19 квіт. (Аліна `34a1201`, Юлія `a781135`, `d899078`) — переважно англійська, дрібні українські залишки у `main.asm:10-12` (Zboard/prevscore/prevbscore) — припустимо. |

### Бонус за розширення: +12.5% (2 повних з меню теми)

| # | Розширення (з теми) | Статус | Доказ |
|---|---|:---:|---|
| 1 | Скасування останнього ходу (збереження попереднього стану поля) | ✅ Повністю | `ctrZ` PROC (`logic.asm:660`) + інфраструктура `Zboard`/`prevscore`/`prevbscore` (`main.asm:9-12`). Ключова тонкість: `prevboard` оновлюється ПЕРЕД кожним ходом у `prevboard_eq_board` (`logic.asm:610`), `Zboard` — ТІЛЬКИ після НЕпустого ходу через перевірку `compare_boards` у `main.asm:179`. Скасування відкочує і поле, і `curr_score`, і `best_score`. Архітектурне рішення з двома знімками. [висока] |
| 2 | Режим сітки 5x5 або 3x3 | ⚪ Частково | `board_type EQU 4` (`main.asm:13`), `board DB 25 DUP(0)` (запас на 5x5). Усі цикли через `board_type*board_type` (`logic.asm:401, 504, 532, 561, 587, 646`). `draw_board` центрується через `find_start_row`/`find_start_col` (`render.asm:431-481`) за формулою `(25 - board_type*(TILE_HEIGHT+1) - 1) / 2`. Усе масштабується на 3x3/4x4/5x5, але потребує перекомпіляції (EQU не runtime-вибір). [середня] |
| 3 | Анімація зсуву | ❌ | Плитки оновлюються миттєво після зсуву (`call draw_board` у `main.asm:182`). |
| 4 | Збереження рекорду через файловий ввід/вивід | ❌ | `best_score` тримається в пам'яті, не зберігається між запусками. |
| 5 | Відстеження найкращої плитки | ❌ | Зберігається `best_score` (рахунок), не максимальна плитка. |
| 6 | Продовження гри після 2048 | ⚪ Частково | `win_triger` (`main.asm:52`) робить win-повідомлення одноразовим, гра продовжується (клавіша C, `main.asm:122-123`). Але `tile_colors` має лише 12 записів (до 2^11=2048), а `merge_row` робить `inc byte ptr [si]` без перевірки межі 11 — після злиття 2048+2048 індексація `tile_colors[12]` виходить за межі таблиці. Розширення таблиці кольорів та обробку більших чисел не зроблено. [низька] |

**Підсумок:** 1×7.5 (Undo, висока) + 1×5 (динамічний розмір, середня) + 0 (continue — частково низька, 1.25% округлено) = **+12.5%**

### Бонус за рівний внесок: +10%

**Зведення за 7 КТ (smart LOC з історії комітів + відео-баланс):**

| Студент | GitHub-alias(-и) | Smart LOC | Відео (КТ) | Частка |
|---|---|---:|---:|---:|
| Паращій Аліна | alina8anila (Alina8anila) | 1080 | 4/7 (КТ-1, КТ-3, КТ-5, КТ-7) | — |
| Сахарова Юлія | Juli-s07 (Julia Sakharova) | 1431 | 4/7 (КТ-2, КТ-4, КТ-6, КТ-7) | — |
| **Ratio_code** | | | | **0.755** |
| **Ratio_video** | | | | **1.000** |

**Висновок:** Ratio_code 0.755 (≥0.7) + Ratio_video 1.000 (≥0.7) → **+10%**. Юлія має ~32% більше smart LOC переважно через нетривіальний `render.asm` з адаптацією під динамічний `board_type` (формули центрування, modulo на `TILE_WIDTH+1`/`TILE_HEIGHT+1`) та два повних ігрових екрани (`draw_game_over`, `draw_win` по ~80 рядків кожен). Аліна реалізувала всю ігрову логіку (slide×4, compress, merge, spawn, check_win/over, copy/compare_boards, Undo). Внесок збалансований за обсягом і за відео.

**Підсумок:** Фінальний код 24/25. Бонус розширень +12.5%. Бонус рівного внеску +10%. (Захист — живий, окремо.)
