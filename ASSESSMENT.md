...
|----------|:----:|----------|
| Наявність обов'язкових секцій (§5.2) | 4/4 | Усі шість секцій присутні: «Опис проєкту», «Аналіз підходів (A проти Б)», «Схема організації пам'яті», «Використані концепції» (регістри, стек, переривання, відеопам'ять, сегменти .STACK/.DATA/.CODE), «Ретроспектива», «План роботи» (розподіл пари). |
| Опис + концепції + схема пам'яті | 4/4 | Опис детальний (механіка, керування R/ESC/Z/C, умови win/lose, динамічний розмір 3x3/5x5). Схема пам'яті зразкова — таблиця зсувів 0000h..01B0h для 23 змінних + діаграма стекового фрейму ([bp+6]/[bp+4]/[bp+2]/[bp+0]/[bp-2]) + опис основних змінних (board, tile_colors, hint_table, curr_score, game_phase). Концепції окремо розписані: регістри за ролями, стек як save/pass, int 16h/21h, відеопам'ять з формулою (row*80+column)*2. |
...
| Ретроспектива | 3/4 | Присутні обидві ретроспективи. Юлія — детально (PR workflow, витік пам'яті через забутий pop, динамічний рендеринг через формули рамки, незавершена анімація зсуву, планування часу). Аліна — коротший абзац (розподіл backend/frontend, одночасне редагування main, 1- vs 2-байтні змінні, відкладена перевірка пустого ходу), але покриває труднощі + що зробила б інакше. |
| Розподіл роботи (пара/соло) | 2/2 | «План роботи» у README чітко розподіляє: Паращій Аліна → Ігрова логіка (compress/merge/slide, spawn_tile, check_game_over/win, Undo, INT 21h); Сахарова Юлія → Рендеринг та ввід (draw_board/draw_tile, кольори, обробник клавіатури, draw_score). |
...
- [Звіт Originality.ai](https://app.originality.ai/share/79ctni0m1zrukedo)

**Вердикт:** ✅ Самостійна робота

_Попередня перевірка (2026-04-23): AI 1.8%, [Originality.ai](https://app.originality.ai/share/toyhzsxblnp5dice)._
